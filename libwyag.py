...
def cmd_log(args):
    ...
    try:
        ...
        # print("  /* No commits found: {} */".format(str(e)))
...
def gitconfig_user_get(config):
    if "user" in config:
        if "name" in config["user"] and "email" in config["user"]:
            return "{} <{}>".format(config["user"]["name"], config["user"]["email"])
    return "Default Author <default@example.com>"
...
#                            tree,
#                            object_find(repo, "HEAD", follow=False),
...
def cmd_commit(args):
    ...
    parent = object_find(repo, "HEAD", follow=False)
    if parent is None:
        # Initial commit case: no parent
        parent = None
